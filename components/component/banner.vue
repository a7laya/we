<template>
  <div style="position: relative;" class="vuxSlider" :style="backimg">
    ...
    <confirm ref="abc" :title="'上传广告图'" @on-confirm="onConfirm" v-model="form.show">
      <vue-cropper  :autoCropWidth="320" :autoCropHeight="180">
        <i class="iconfont icon-tsh-copy" v-if="!form.imgurl" style="font-size: 40px;"></i>
        <img ref="aaa" class="showimg" :src="$store.state.website.website_domain_name + '/uploads/' + form.imgurl" alt="" v-else/>
      </vue-cropper>
    </confirm>
  </div>
</template>
...
//    console.log(this.$refs.abc)
    ...
    methods: {
      ...
      go () {
        ...
        if (this.banner_list[this.banner_index].title == '生成我的广告') {
