<template>
	<!--
    	...
    	描述：地区选择
    -->
	<popup-picker ref="picker" :placeholder="placeholder" :title="title" :data="item" v-model="add" :show-name="true" :columns="3" @on-hide="hide"></popup-picker>
</template>
...
<script>
    ...
    export default {
        ...
        mounted() {
            var _this = this;
            _this.$http.post(_this.$store.state.url + '/Common/hangye').then(function(res) {
                if(!res) return;
                if(_this.show){
