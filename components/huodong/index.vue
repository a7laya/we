<template>
	<div>
		<x-header :title="'活动列表'" :left-options="{backText:'',preventGoBack:true}" class="header" @on-click-back="fanhui()">
			<div slot="right">
				<vue-header-nav></vue-header-nav>
			</div>
		</x-header>
		<vue-swiper :type="7"></vue-swiper>
		...
		<div class="index_box" style="padding-top: 2.4rem;">
			<vue-list1 v-for="(item,index) in list" :type="2" :item="item" :type2="index" :key="index" @zhiding="zhiding"></vue-list1>
			<vue-loading :url="$store.state.url + '/Activityb/getAllAct?page=1&limit=10&region='+region+'&industry='+hangye" @ievent="loaddata" v-if="isshow"></vue-loading>
		</div>
		<vue-foot></vue-foot>
...
<script>
	import { XHeader, Badge, PopupPicker, XDialog, TransferDom,XButton } from 'vux'
	import { VueLoading, VueSwiper, VueFoot, VueSuspen, VueList1, VueHeaderNav, VueShareit, } from '../component'
	export default {
		components: {
			XHeader,
			VueLoading,
      ...
      initHeight () {
        ...
        console.log(_this.add)
			...
			sureDing() {
				let _this = this;
				_this.$http.post(_this.$store.state.url + '/Activityb/act_top', {
					load: false,
					act_id: _this.listId,
				}).then(function(res) {
