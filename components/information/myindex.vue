<template>
  <div>
    <x-header :title="'我的资讯'" :left-options="{backText:''}">
      <div slot="right">
        <vue-header-nav></vue-header-nav>
      </div>
    </x-header>
    <tab>
      <tab-item selected @on-item-click="onItemClick(1)">新闻发布</tab-item>
      <tab-item @on-item-click="onItemClick(2)">新品发布</tab-item>
    </tab>
    <!-- 新闻 开始 -->
    <div id="divs"></div>
    <template v-if="index==1">
      <div v-for="(item,index) in list">
        <vue-list :type="4" :item="item" :key="index">
          <div class="remove_butt">
            <router-link :to="'/information/newedit/' + item.id" tag="span">
      ...
      <vue-loading :url="$store.state.url + '/Information/select_uid_release?type=2&page=1&limit=10'" @ievent="loaddata"></vue-loading>
    </template>
    <!-- 新闻 结束 -->
    ...
    <template v-if="index==2">
      <!---->
      <div v-for="(item,index) in list">
        <vue-list :type="5" :item="item" :key="index">
          <div class="remove_new">
            <router-link :to="'/information/newpedit/' + item.id" tag="span">
              <i class="iconfont icon-jilu"></i>
              <span class="span">编辑</span>
            </router-link>
          </div>
      ...
      <vue-loading :url="$store.state.url + '/Information/select_uid_release?type=3&page=1&limit=10'" @ievent="loaddata"></vue-loading>
    </template>
    <!-- 新品 结束 -->
