<template>
  <div style="background: #fff;">
    <x-header class="header">
      ...
      <div slot="overwrite-title" class="title">编辑新闻</div>
    </x-header>
    <div class="top">
      <x-input class="weui-vcode" placeholder="编辑发布资讯标题" v-model="news.news_name">
      ...
      <div class="ImgButton">
        ...
        <div class="img-box user_up_imgfile">
         <span v-for="(item, index) in news.news_imgs" :key="index">
          ...
          <vue-cropper @data="fileImgarr" :autoCropWidth="320" :autoCropHeight="180" v-show="news.news_imgs.length < 1">
    ...
    methods: {
      getData() {
        ...
        _this.$http.post(_this.$store.state.url + '/Information/infoDetails', {
          ...
          if (!res) return;
          _this.news.news_imgs = res.imgUrl
          _this.news.news_name = res.title
          _this.news.news_details = res.content
        })
      },
      deletePic(index) {
        this.show = true;
        this.suoyin = index;
        this.news.news_imgs.splice(index, 1);
      },
      fileImgarr(res) {
        this.news.news_imgs.push(res.imgurl);
      },
      saveProjectImg() {
        var _this = this;
      ...
      saveZZ() {
        ...
        _this.$http.post(_this.$store.state.url + "/Information/update_information", {
            ...
            imgUrl: _this.news.news_imgs,
            content: _this.news.news_details
          })
          .then(function(res) {
            msg("编辑成功");
