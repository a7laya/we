<template>
	<div style="background: #fff;">
		 <x-header class="header">
		  ...
		  <div slot="overwrite-title" class="title" >发布新品</div>
		</x-header>
		<div class="top">
			<x-input  class="weui-vcode" placeholder="输入发布新品标题" v-model="project.project_name"  >
			...
			<div class="xiangqing">
				...
				<span class="xiangqing-wen">编辑框内输入新品图文</span>
			</div>
		...
		<div v-transfer-dom>
			...
			<x-dialog :show.sync="enterpriseImgs3" class="investMoney">
				...
				<div class="img-box user_up_imgfile">
					...
					<vue-cropper @data="fileImgarr" :autoCropWidth="320" :autoCropHeight="180" v-show="imgUrl.length < 2">
		...
		<div v-transfer-dom style="position:relative">
			<x-dialog v-model="show2" class="dialogHangye" :hide-on-blur="true">
				<div style="padding:15px;" class="dialogHangyeBox">
					<div class="dialoghui">
						<div>使用10个智汇币 发布展示新品信息</div>
					</div>
					<!-- <x-button :type="primary" mini @click.native="onHide" class="makeSureHY">确定</x-button> -->
	...
	export default{
        ...
        if(!_this.project.project_name) {
        	...
        	return;
				...
				if(_this.project.project_imgs.length < 2) {
					msg("请上传2张图片");
					return;
				}
				if(!_this.project.project_details){
					msg("请输入新品详情");
					return;
				}
				_this.show2=true
			 ...
			 onHide () {
				...
				_this.$http.post(_this.$store.state.url + "/Information/newsRelease", {
						...
						img:_this.project.project_imgs,
						info:_this.project.project_details
					})
					.then(function(res) {
						if(res == '余额不足，请充值！'){
							_this.show2=false
							...
							_this.$router.push('/information/index/' + '新品展示')
							_this.show2=false
						}
