<template>
	<div>
		<x-header title="关联的甲方" :left-options="{backText:''}" class="header"></x-header>
		<!--基本信息-->
		<div class="info">
			<div class="info_top">
				<div>招标代理：</div>
			...
			<div class="list_item">
				...
				<div class="item_left">
					...
					<div class="left_info">
						...
						<div>关联企业：<span>{{item.re_com}}</span>家</div>
					</div>
				</div>
			</div>
			<div class="right">
		...
		<vue-loading :url="$store.state.url + '/Collection/biddingAgentHistoryA?page=1&limit=10&agent_id='+$route.query.id" @ievent="loaddata" v-if="isshow"></vue-loading>
