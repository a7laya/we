<template>
	<div style="background:#fff">
		<!-- <x-header title="关联的代理机构" :left-options="{backText:''}" class="header"></x-header> -->
		<!-- <x-header title="历史招标代理机构记录" :left-options="{backText:''}" class="header"></x-header> -->
		
		<div class="header-search">
		    <div class="search-content">
				<div class="fanhui" @click="back($route.query.id,$route.query.is_error)"><img src="/static/img/fanhui.png"></div>
		        <div class="searchbox">
		           历史招标代理机构记录
		        </div>
		    </div>
		...
		<div class="info">
			<div class="info_top">
				...
				<!-- <div><span style="color: #01B0B7">招标单位:</span>&#x3000;{{$route.query.des}}</div> -->
		...
		<!--列表-->
		<div class="list" @click="detail(item.id)" v-for="(item,index) in fang" :key="index">
		<!-- <div class="list" v-for="(item,index) in fang" :key="index"> -->
			<div class="list_item">
				...
				<div class="item_left">
					<div class="left_info">
						...
						<div>代理服务: <span>{{item.agent_bidding}}</span>条</div>
					</div>
					<div class="left_info">
						...
						<div>关联甲方：<span>{{item.relevant_party_a}}</span>家</div>
					</div>
				</div>
			</div>
			<div class="right">
		...
		<vue-loading :url="$store.state.url + '/Collection/historyBiddingAgent?page=1&limit=10&pId='+$route.query.id" @ievent="loaddata" v-if="isshow"></vue-loading>
...
<script>
	...
	export default {
		...
		mounted() {
			let _this = this;
			_this.$http.post(_this.$store.state.url+'/Collection/historyBiddingAgent',{
...
// 				_this.fang=res
		...
		methods: {
           ...
           detail(id){
           	let _this=this;
           	_this.$router.push("dialijigou?id="+id)
           },
		   ...
		   back(id,is_error){
			   // this.$router.push("/project/subject/?id="+id+"&type=商机分析")
