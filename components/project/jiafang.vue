<template>
	<div>
		<x-header title="历史招标甲方记录" :left-options="{backText:''}" class="header"></x-header>
		<!--基本信息-->
		<div class="info">
			<div class="info_top">
				<div>中标单位：</div>
				<div>{{$route.query.cen}}</div>
				<!-- <div>+关注</div> -->
			...
			<div class="info_bottom">
				<div>企业所在地:{{$route.query.dree}}</div>
				<!-- <div>企业主页</div> -->
			</div>

		</div>
		<!--列表-->
		<div class="list" @click="detail(item.id)" v-for="(item,index) in list" :key="index">
			<div class="list_item">
				<div class="item_name"><span class="red"></span>{{item.company}}</div>
				<div class="item_left">
					<div class="left_info">
						...
						<div>招采信息: <span>{{item.bid_num}}</span>条</div>
					</div>
					<div class="left_info">
						...
						<div>关联企业：<span>{{item.re_win_com}}</span>家</div>
					</div>
				</div>
			</div>
			<div class="right">
		...
		<vue-loading :url="$store.state.url + '/Collection/historyWin?page=1&limit=10&win_company_id='+$route.query.id" @ievent="loaddata" v-if="isshow"></vue-loading>
...
<script>
	...
	import { VueMessage,VueLoading,VueDingyue,VueFoot, } from '../component/'
	export default {
		components: {
			XHeader,
			VueMessage,
		...
		mounted() {
			let _this = this;
			_this.business();
			 /*_this.$http.post(_this.$store.state.url + "/Collection/historyWin",{
