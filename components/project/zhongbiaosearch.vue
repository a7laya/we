		...
		<div class="header-search">
		    <div class="search-content">
		        ...
		        <div class="button_fabu fl">
		        	<vue-zhongbiaoxuan ref="sfilter" @ievent="ievent">筛选</vue-zhongbiaoxuan>
		        </div>
		    </div>
			<vue-message :type="1" v-for="(item,index) in list" :item="item" :key="index"></vue-message>
			<vue-loading v-if="url" :url="$store.state.url + '/Collection/winningList?page=1&limit=10&' + url" @ievent="loaddata"></vue-loading>
		</div>
	</div>
</template>
...
<script>
	...
	import { VuePosition,VueCaixuan,VueMessage,VueLoading,VueZhongbiaoxuan} from '../component/'
	export default{
		components: {
		    VuePosition,
		    VueCaixuan,
