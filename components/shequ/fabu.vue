<template>
	<div id="up">
		<x-header :title="'发布'" :left-options="{backText:''}" class="header"></x-header>
		<div>
		...
		<div class="sctp">
			...
			<div id="upload" v-if="ImgList.length<9">
				<!-- 裁剪图片 -->
				<!-- <vue-cropper1 @data="fileimg" :enlarge="3">
					<img src="/static/img/jh.png" class="tp" />
				</vue-cropper1> -->
				<!-- <img v-if="src" :src="$store.state.website.website_domain_name + '/uploads/' + src"> -->
				<form enctype="multipart/form-data" method="post" id="upload">
					<img src="../../../static/img/jh.png" alt="" class="tp" />
					<input v-if="type==1" type="file" name="upload" capture="camera" accept="image/*" @change="doUpload1($event)"
					 class="upload1" style="width: 100%; height: 100%;  opacity: 0;position: absolute;top: 0;left: 0;">
					...
					<input v-if="type==3" type="file" name="upload" accept="image/*" multiple @change="doUpload1($event)" class="upload1"
