<template>
  <div class="wrap">
    <!-- header -->
    <x-header :left-options="{backText:''}" style="background: #398CD3;color: #000000!important;"><span>编辑尾货</span></x-header>
    <!-- list -->
    <div class="nav">
      <div class="nav_top">
        <span>名称:</span>
      ...
      <div class="nav_tab">
        ...
        <div>
          ...
          <div class="param">
          <input type="text" class="input1" v-model="input1">&nbsp;:&nbsp;<input type="text" class="input2" v-model="input2">
...
<script>
  ...
  import catePicker from '../component/brand-picker'
  export default {
    data() {
      return {
    ...
    methods: {
      ...
      getCargo() {
          ...
          console.log('接受点击编辑传过来id:',id);
          if (!id) {
            this.$router.go(-1)
          }
          var _this = this
          this.$http.post(this.$store.state.url + "/Cargo/find_goods_details", {
              goods_id: id
          }).then(function(res) {
              if (!res) return;
              console.log('Cargo/find_goods_details尾货编辑页面获取到的商品详情:',res);
              _this.goods_id = res.goods_id;
              _this.goods_name = res.goods_name;
              _this.goods_img = res.imgs;
              _this.goods_state = [res.goods_state];
              // 获取地址代码
              _this.region = res.region
              _this.region_name = res.region_name
              // 获取行业分类代码
              _this.cate_id = `-${res.hangye_id}-${res.cat_id}-`
              _this.cat_id = res.cat_id
              _this.cat_name = res.cat_name
              _this.hangye_id = res.hangye_id
              _this.hangye_name = res.hangye_name
              _this.shipping_type = [_this.shipping_types[0][res.shipping-1]];
              _this.freight = res.freight;
              _this.before_price = res.market_price;
              _this.price = res.shop_price;
              _this.shop_num = res.store_count;
              _this.goods_content = res.goods_content;
              var parameter = res.parameter
              console.log('parameter:',parameter);
              if(parameter.length > 0){
                Object.keys(parameter[0]).forEach(function(key){
                     _this.input1 = key
                     console.log('_this.input1:',_this.input1);
                     _this.input2 = parameter[0][key]
                     console.log('_this.input2:',_this.input2);
                })
              }
              if(parameter.length  > 1){
                Object.keys(parameter[1]).forEach(function(key){
                     _this.input3 = key
                     _this.input4 = parameter[1][key]
                })
              }
              if(parameter.length  > 2){
                Object.keys(parameter[2]).forEach(function(key){
                     _this.input5 = key
                     _this.input6 = parameter[2][key]
                })
              }
              if(parameter.length  > 3){
                Object.keys(parameter[3]).forEach(function(key){
                     _this.input7 = key
                     _this.input8 = parameter[3][key]
                })
              }
              if(parameter.length  > 4){
                Object.keys(parameter[4]).forEach(function(key){
                     _this.input9  = key
                     _this.input10 = parameter[4][key]
                })
              }
      ...
      setData() {
        ...
        _this.$http.post(_this.$store.state.url + '/Cargo/edit_cargo', {
          ..._this.form
        }).then(function(res) {
          if(!res) return;
          msg('发布成功')
