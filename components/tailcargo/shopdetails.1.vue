<template>
  <div class="wrap">
    <vue-shop-header-nav></vue-shop-header-nav>
    <!-- 轮播图 -->
    ...
    <swiper loop auto :list="goods_imgs" ></swiper>
    <!-- 商品详情 -->
    <div class="content">
      <div>
      ...
      <div>
        <span>商品编号:{{goods_list.goods_sn}}</span>
        <span style="margin-left: 1.6rem;">{{goods_list.goods_state}}</span>
      ...
      <div class="price">
        ...
        <span class="collect" v-else-if="isCollect==false" @click="noCollect()">已收藏</span>
      </div>
      <div>
        <span>产品所在地</span>
    ...
    <div class="shop_like">
      <span>猜你喜欢</span>
      <div class="shop_list">
        <div v-for="(like,index) in goods_like" :key='index'  @click="shopDetails(like.goods_id)">
          <img :src="$store.state.website.website_domain_name + '/uploads/' + like.img_name" alt=""></br>
          <span>{{like.goods_name}}</span></br>
          <span style="color: #FF6565;">￥{{like.shop_price}}</span>
        </div>
...
<script>
  ...
  export default {
    ...
    watch: {
      'all_num': {
        handler() {
          this.all_money = this.all_num * this.goods_list.shop_price + this.goods_list.freight + '.00'
        },
        immediate: false
      }
    },
