<template>
  <div>
    <x-header :title="'历史发布'" :left-options="{backText:''}">
      <div slot="right">
    ...
    <template v-if="index==1">
      <div v-for="(item,index) in list">
        <vue-list :type="1" :item="item" :key="index">
          <div class="remove_butt">
            <!--<span @click.stop="remove(index)"><i class="iconfont icon-shanchu"></i><span class="span">删除</span></span>-->
            <router-link :to="'/hezuo/edit/' + item.id" tag="span"><i class="iconfont icon-jilu"></i><span class="span">编辑</span></router-link>
          </div>
        </vue-list>
      ...
      <vue-loading :url="$store.state.url + '/Homecenter/historyRelease?type=1&page=1&limit=10'" @ievent="loaddata"></vue-loading>
    </template>
    <!-- 合作 结束 -->
    ...
    <template v-if="index==2">
      ...
      <div v-for="(item,index) in list">
        <vue-list :type="2" :item="item" :key="index">
          <div class="remove_butt" v-if="item.sign_end_time < dangqian_time || item.process == 1">
            <!--<span @click.stop="remove(index)"><i class="iconfont icon-shanchu"></i><span class="span">删除</span></span>-->
            <router-link v-if="item.isup == 0 " :to="'/huodong/edit/' + item.id" tag="span"><i class="iconfont icon-jilu"></i><span class="span">编辑</span></router-link>
          </div>
        </vue-list>
        <div v-if="item.status==2" style="background: gainsboro; color: red;line-height: 25px; height: 25px;font-size: 14px;">
          <marquee scrollamount="3">审核失败原因：{{item.reason}}</marquee>
