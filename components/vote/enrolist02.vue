<template>
    ...
    <vue-back :name='info.vote_title' :type='1'></vue-back>
		...
		<div class="wrap">
		  <div class="time">
		  	...
		  	<p class="time-p" v-else-if=" 0 >= time">
		...
		<div class="search">
			<input placeholder="请输入选手名称、品牌......" class="txt" v-model="add" onfocus="this.placeholder=''" onblur="this.placeholder='请输入参赛选手名称、品牌......'"/>
		...
		<div class="control">
			<div class="control-frist" @click="skip(item.id,info.vote_end_time,(index+1),info.vote_title,item.number) " v-for='(item,index) in vote_list' :key="index">
				...
				<!-- <div class="control-tou" @click.stop="cast">投票</div> -->
	...
	<vue-loading5 :url="$store.state.url + '/Vote/voteInfo?page=1&limit=10&vote_id='+$route.params.id+'&keyword='+keyword" @ievent="loaddata" v-if="isshow"></vue-loading5>
...
//      },
		mounted() {
			...
			const timer = setInterval(() => {
				this.time = this.info.vote_start_time - (Date.parse(new Date()) / 1000);
				this.signEnd = this.info.vote_end_time - (Date.parse(new Date()) / 1000);
				this.signEnd--
				this.time--
			}, 100);
...
//					_this.vote_list = res.vote_select_list
				})
			},
			loaddata(res) {
				var _this = this;
			...
			skip(id, time, feel, des,num) {
				this.$router.push("/vote/enrodetail/" + id + "/" + time + "/" + feel + "/" + des + '/' + num)
			},
			present(id) {
			...
			meet(id) {
//				console.log(this.vote_select_count)
