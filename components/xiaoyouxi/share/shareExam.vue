<template>
  <div class="hezuo huodong">
    <!-- <transition name="slide-fade">
      <vue-step v-show="!industry && !business" @onClickBack="remindustry" @onClickNext="addindustry"
                :url="'/Applets/hangyes'" :title="'选择所属行业'" :assistId="specialUidkey"></vue-step>
    </transition> -->
    <transition name="slide-fade">
      <div class="container" v-for="(item,index) in list.datas" v-show="!business && industry" v-if="questionIndex == index"
        ...
        <div class="body">
          ...
          <div class="answers">
            <ul>
              <li :class="{success:show == 1 && chooseId == 1, error:show == 2 && chooseId == 1}" class="answer1"
                @click="clickFlag && chooseAnswer(1,item.answer_num)">
                <span>{{item.answer_group[0]}}</span>
              </li>
              <li :class="{success:show == 1 && chooseId == 2, error:show == 2 && chooseId == 2}" class="answer2"
                @click="clickFlag && chooseAnswer(2,item.answer_num)">
                <span>{{item.answer_group[1]}}</span>
              </li>
            </ul>
          </div>
          <div class="adImg">
    ...
    <transition name="slide-fade">
      <div class="businesscontainer" v-show="industry && business">
        ...
        <vue-red :item="list.red" v-model="isShow" :times="times"></vue-red>
      </div>
    </transition>
    <div v-transfer-dom>
...
<script>
  ...
  export default {
    ...
    mounted() {
      var _this = this
      _this.huodong_id = _this.$route.query.huodong_id
    },
    computed: {
      user() {
    ...
    methods: {
      fenxiang() {
        // type为0助力分享
        var _this = this
        var shareDese = '这道题好难啊！' + this.$store.state.user.mem_nickname + '向你求助！快去帮助Ta吧！'
        ...
        if (!_this.helpFlag && !_this.$route.query.specialUidkey) {
          ...
          _this.shareDese = this.$store.state.user.mem_nickname + '邀您免费挑战弱电行业知识竞赛，领现金红包、冲刺夺冠大奖！'
          _this.shareimgUrl = shareimgUrl
          _this.shareLink = '/game/shareExam?specialUidkey=' + this.$route.query.specialUidkey
        } else if (_this.helpFlag && !_this.$route.query.specialUidkey) {
          _this.shareTitle = shareTitle
          _this.shareDese = shareDese
          _this.shareimgUrl = shareimgUrl
          _this.shareLink = '/game/shareExam?fd=' + this.tid + '&sponsor_id=' + this.industry
        } else if (_this.helpFlag && _this.$route.query.specialUidkey) {
          _this.shareTitle = shareTitle
          _this.shareDese = shareDese
          _this.shareimgUrl = shareimgUrl
          _this.shareLink = '/game/shareExam?fd=' + this.tid + '&sponsor_id=' + this.industry + '&specialUidkey=' +
            this.$route.query.specialUidkey
        }
      },
      friendHelp() {
        this.helpFlag = 1
      ...
      addindustry(res) {
        let _this = this
        _this.industry = _this.$route.query.id
        _this.getExamData(_this.industry)
      },
      remindustry() {
        this.$router.back(-1)
      ...
      getExamData(id) {
        ...
        if (!_this.$route.query.sponsor_id) {
          _this.$http.post(_this.$store.state.url + '/Applets/get_sponsor_topic', {
            ...
            if (!res) {
              _this.$router.push('/game/onlyindex?shareId=' + _this.huodong_id)
            } else {
              if (res === '智汇币不足') {
